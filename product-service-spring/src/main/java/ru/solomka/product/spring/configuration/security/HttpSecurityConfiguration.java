...
public class HttpSecurityConfiguration {
    ...
    SecurityFilterChain securityFilterChain(@NotNull HttpSecurity http,
                                            ...
                                            @NonNull OnceRequestFilter onceRequestFilter) throws Exception {
        return http
                .sessionManagement(
                ...
                .authorizeHttpRequests(registry -> registry.requestMatchers("/product/v3/api-docs/**", "/v3/api-docs/**").permitAll()
                        .anyRequest().authenticated()
                ).exceptionHandling(configurer -> configurer
                        .authenticationEntryPoint(authenticationEntryPoint)
                        .accessDeniedHandler(accessDeniedHandler)
                ).addFilterBefore(onceRequestFilter, UsernamePasswordAuthenticationFilter.class).build();
    }
